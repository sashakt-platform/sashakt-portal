<script lang="ts">
	...
	function updateOptionLabel(idx: number, newLabel: string) {
		options = options.map((opt, i) => {
			if (i === idx) {
				const previousAutoSlug = toSlug(opt.label);
				const newAutoSlug = toSlug(newLabel);
				const valueWasAutoGenerated = opt.value === '' || opt.value === previousAutoSlug;
				return {
					...opt,
					label: newLabel,
					value: valueWasAutoGenerated ? newAutoSlug : opt.value
				};
			}
			return opt;
		});
